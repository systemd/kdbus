...
/**
 * kdbus_node_pre_recurse_unlock() - advance iterator on a tree
 * @root:	node at which the iteration started
 ...
 *             mutex_lock(&n->lock);
 *             ... visit @n ...
 *             n = kdbus_node_pre_recurse_unlock(root, n);
 *     }
 *
 ...
 * hold a reference to @root during the whole iteration. Furthermore, @node
 ...
 */
static struct kdbus_node *kdbus_node_pre_recurse_unlock(struct kdbus_node *root,
...
/**
 * kdbus_node_post_recurse_unlock() - advance iterator on a tree
 * @root:	node at which the iteration started
 * @node:	previously visited node, or NULL on start
 *
 * This is similar to kdbus_node_pre_recurse_unlock(), but implements
 * post-order traversal. It's supposed to be used like this:
 *
 *     struct kdbus_node *n;
 *
 *     n = kdbus_node_post_recurse(root, NULL);
 *     while (n) {
 *             ... visit @n ...
 *             n = kdbus_node_post_recurse(root, n);
 *     }
 *
 * Unlike kdbus_node_pre_recurse_unlock(), this must be called without any
 * locks held. For detailed usage notes, see kdbus_node_pre_recurse_unlock().
 *
 * Return: Reference to next node, or NULL.
 */
static struct kdbus_node *kdbus_node_post_recurse(struct kdbus_node *root,
						  struct kdbus_node *node)
{
	struct kdbus_node *parent;
	struct rb_node *rb;

	if (node == root) /* we're done */
		return NULL;

	if (!node) /* on first iteration, find left-most child */
		parent = root;
	else /* on next iteration, find sibling or visit parent */
		parent = node->parent;

	mutex_lock(&parent->lock);
	rb = node ? rb_next(&node->rb) : rb_first(&parent->children);
	if (!rb) { /* no next-sibling / first-child, visit the parent */
		mutex_unlock(&parent->lock);
		kdbus_node_unref(node);
		return parent; /* parent already ref'red by previous run */
	}

	do { /* find left-most child to visit */
		node = kdbus_node_ref(kdbus_node_from_rb(rb));
		mutex_unlock(&parent->lock);
		parent = node;
		mutex_lock(&parent->lock);
		rb = rb_first(&parent->children);
	} while (rb);
...
void kdbus_node_deactivate(struct kdbus_node *node)
{
	...
	while (pos) {
		...
		pos = kdbus_node_pre_recurse_unlock(node, pos);
	}
}
